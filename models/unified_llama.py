    ...
    def forward(
        ...
        **kwargs
        ...
        if self.is_avs_task:
            ...
            # avs_hidden_states = last_hidden_states.detach()
            # avs_hidden_states = last_hidden_states[avs_batch_mask]
            # avs_nums, seq_len, dim = avs_hidden_states.shape
            bs, seq_len, dim = avs_hidden_states.shape
            pred_embeddings = avs_hidden_states[mask_token_mask]
            pred_embeddings = pred_embeddings.reshape(bs,-1,dim)
            # print('batch_task_names: ',batch_task_names, ' pred_embeddings.shape: ',pred_embeddings.shape)
